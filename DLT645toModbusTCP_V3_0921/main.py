    ...
    def Recive645Data(self):        # 将645数据转换成modbus TCP数据发送到平台
        ...
        while True:
            ...
            if recv_count > 0 :
                ...
                modbus_tcp_engine.con.send(mb_resp_bytes) #上行发送命令到JAVA端     # modbus_tcp_engine.con在Communication_MBTCP类中定义
            ...
            if addr645 != "" and ctrl == WriteData and di_data != None:      #如果数据转换成功
                ...
                if di_num == 1:
                    ...
                    while i < di_num:
                        ...
                        di_temp3 = bytearray.fromhex(b[i][3].zfill(2))  # 写线圈的时候，每位写一位数据，不符合16进制字符，进而组帧不成功，所有将不足2位的数据进行高位补0；但是写寄存器就不用，已经满足了2位数据
                        di_bytearry.extend(di_temp0)
                        di_bytearry.extend(di_temp1)
                        di_bytearry.extend(di_temp2)
                        di_bytearry.extend(di_temp3)
...
if __name__ == '__main__':
   ...
   #modbus_tcp_engine = Communication_MBTCP('127.0.0.1',9090,5)
    modbus_tcp_engine = Communication_MBTCP('10.1.10.77',10000,5)
