...
class dlt_645_2007:
    ...
    def decode_645data_shuzu(self,data):
        decode_data = []
        val = 0xff
        for i in data:
            if i > 0x32:
    ...
    def decode_645data_unit(self,data): #减33操作 解析数据
        val = 0xff
        if data > 0x32:
            val = data - 0x33
        return val
    
    def encode_645data_unit(self,data): #加33操作 组帧传送
    ...
    def decode(self, data): #接收时用字节序列接收
        try:
            logger.info(binascii.hexlify(data))
            print("--------------------开始遍历数组，解析数据----------------------")
            for temp in data:
                if self.st == ST_IDLE:                                    #开始接收标志
                    if temp == START_MARK:                                #接收到第一个转变浮标
                    ...
                    self.idx += 1
                    if self.idx == 10:    
                        if self.cache[7] != START_MARK:                     #接收错误，浮标置1重新开始接收
                            self.idx = 1                   
                            self.cache = bytearray(0)
                            self.cache.append(START_MARK)
                        else:
                            self.addr = bytearray(self.cache[1:7])           #接收正确，提取属性
                            self.frame_size = self.cache[9] + 12             #总帧长度 十个固定数据加一个校验码、一个帧尾
                            self.st = ST_FRAME             
                else:                                                        #这里开始接收数据域
                    self.cache.append(temp)
                    self.idx += 1
                    if self.idx == self.frame_size:                          #接收完毕
                        ...
                        if crc == self.cache[-2] and self.cache[-1] == 0x16: #数据接收无误，开始获取数据组返回帧
                            ...
                            temp_data = self.cache[14:-2] 
                            list_data = list(temp_data)
                            self.di = self.decode_di()                        #数据域减33，获取到数据标志
                            self.data = self.decode_645data_shuzu(list_data)  #数据域减33，获取到设备上传的数据
